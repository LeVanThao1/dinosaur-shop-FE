    ...
    useEffect(() => {
        ...
        API("api/orders", "GET", null, token).then((res) => {
            ...
            for(let i =0 ; i< temp.length;i++){
                ...
                obj["date"] = moment(temp[i].dateOrder).format("DD/MM/YYYY");
                obj["status"] = temp[i].status===0?"đang chờ lấy hàng":temp[i].state ===1?"đang giao":temp[i].status===2?"đã giao":"chờ xác nhận";
                obj["pay"] = (temp[i].status===2 || temp[i].typePayment===1)?"đã thanh toán":"chưa thanh toán";
